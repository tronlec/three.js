...
(function () {
    ...
    THREE.TransformControls = function ( camera, canvas, eventSource ) {
        ...
        eventSource.addEventListener( "mousedown", onPointerDown, false );

        eventSource.addEventListener( "mousemove", onPointerHover, false );

        eventSource.addEventListener( "mousemove", onPointerMove, false );

        eventSource.addEventListener( "mouseup", onPointerUp, false );
        eventSource.addEventListener( "mouseout", onPointerUp, false );
        ...
        function onPointerHover( x, y ) {

            if ( scope.object === undefined || _dragging === true ) return;

            var intersect = intersectObjects( x, y, _gizmo[_mode].pickers.children );
        ...
        function onPointerDown( x, y, buttons ) {

            if ( scope.object === undefined || _dragging === true ) return;

            if ( buttons === THREE.MOUSE.LEFT ) {
                ...
                if ( intersect ) {
                    ...
                    var planeIntersect = intersectObjects( x, y, [ _gizmo[_mode].activePlane ] );
                    if ( planeIntersect ) {
        ...
        function onPointerUp( posX, posY ) {
            ...
            _dragging = false;
            onPointerHover( posX, posY );

        }

        function intersectObjects( posX, posY, objects ) {

            var x = ( posX ) / canvas.width;
            var y = ( posY ) / canvas.height;
